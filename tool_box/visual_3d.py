...
@torch.no_grad()
def stream_line(XY_meshgrid,model,pic_name="stream.png",resol=(100,100),use_f64=False):
    X, Y = XY_meshgrid
    Z = np.zeros_like(X); Z[:,:] = 0.1
    ntype = np.float64 if use_f64 else np.float32
    XYZ = np.stack([X.ravel(), Y.ravel(), Z.ravel()], axis=1).astype(ntype)
...
def speed_l2_error(
    ...
    xy_np      : numpy.ndarray, 形状 (N,3)，分别为 (x,y)
    uv_np      : numpy.ndarray, 形状 (N,3)，分别为 (u,v) 真值
    ...
    # numpy -> torch
    xyz_t = torch.from_numpy(xyz_np).to(device=device, dtype=dtype)
    uv_pred_t = model(xyz_t)[:,0:2]
